...
export const ListsComponent: React.FC<IListsComponentProps> = ({
  ...
  return (
    <Box gap={1} marginX={1} padding={1} paddingX={2} display="flex" alignItems="center" height={theme.spacing(5)} component={Paper}>
      {showSearchInput && <TextField size="small" value={searchText} placeholder={searchPlaceholder} onChange={(e) => onSearchTextChange?.(e.target.value)} />}
      ...
      <Box flex={1} display="flex" justifyContent="end">
        {showDownloadButton && (
          <Button color="primary" disableElevation variant="outlined" onClick={onDownloadButtonClick} endIcon={<Icon>download</Icon>}>
            Download
          </Button>
        )}
      </Box>
      ...
      <Box display="flex" justifyContent="end">
        {showNewButton && (
          <Button color="primary" disableElevation variant="contained" onClick={onNewButtonClick} endIcon={<Icon>add</Icon>}>
            {newButtonLabel}
          </Button>
        )}
      </Box>
